...
export class BasicTranscriptSyncProvider implements TranscriptSyncProvider {
  async autoSync(
    ...
    const sliceMs = Math.max(1000, Math.floor((audio.durationSeconds * 1000) / segments.length));
    return segments.map((segment, index) => {
      const syncedSegment: SyncedSegmentDraft = {
        segmentId: segment.segmentId,
        startMs: index * sliceMs,
        endMs: index === segments.length - 1 ? audio.durationSeconds * 1000 : (index + 1) * sliceMs,
